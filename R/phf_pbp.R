...
NULL
#' @title
#' **Load cleaned PHF play-by-play from the data repo**
#' @rdname load_phf_pbp
...
load_phf_pbp <- function(seasons = most_recent_phf_season(),...,
  ...
  urls <- paste0("https://raw.githubusercontent.com/saiemgilani/fastRhockey-data/main/phf/pbp/rds/play_by_play_",seasons,".rds")
...
#' @name load_phf_team_box
NULL
#' @title
#' **Load cleaned PHF team box scores from the data repo**
...
load_phf_team_box <- function(seasons = most_recent_phf_season(), ...,
  ...
  urls <- paste0("https://raw.githubusercontent.com/saiemgilani/fastRhockey-data/main/phf/team_box/rds/team_box_",seasons,".rds")
...
#' @name load_phf_player_box
NULL
#' @title
#' **Load cleaned PHF player box scores from the data repo**
...
load_phf_player_box <- function(seasons = most_recent_phf_season(), ...,
  ...
  urls <- paste0("https://raw.githubusercontent.com/saiemgilani/fastRhockey-data/main/phf/player_box/rds/player_box_",seasons,".rds")
...
#' @name load_phf_schedule
NULL
#' @title
#' **Load cleaned PHF schedules from the data repo**
...
load_phf_schedule <- function(seasons = most_recent_phf_season(), ...,
  ...
  urls <- paste0("https://raw.githubusercontent.com/saiemgilani/fastRhockey-data/main/phf/schedules/rds/phf_schedule_",seasons,".rds")
...
# load games file
load_phf_games <- function(){
  .url <- "https://raw.githubusercontent.com/saiemgilani/fastRhockey-data/main/phf/phf_games_in_data_repo.csv"
  con <- url(.url)
...
update_phf_db <- function(dbdir = ".",
  ...
  rule_header("Update fastRhockey PHF Play-by-Play Database")
  ...
  if (!DBI::dbExistsTable(connection, tblname)) {
    build_phf_db(tblname, connection, rebuild = "NEW")
  } else if (DBI::dbExistsTable(connection, tblname) & all(force_rebuild != FALSE)) {
    build_phf_db(tblname, connection, rebuild = force_rebuild)
  }

  # get completed games
  user_message("Checking for missing completed games...", "todo")
  completed_games <- load_phf_games() %>%
    # completed games since 2002, excluding the broken games
    dplyr::filter(.data$season >= 2016) %>%
    dplyr::pull(.data$game_id)

  # function below
  missing <- get_missing_phf_games(completed_games, connection, tblname)

  # rebuild db if number of missing games is too large
  if(length(missing) > 100) {
    build_phf_db(tblname, connection, show_message = FALSE, rebuild = as.numeric(unique(stringr::str_sub(missing, 1, 4))))
    missing <- get_missing_phf_games(completed_games, connection, tblname)
  }
...
build_phf_db <- function(tblname = "fastRhockey_phf_pbp", db_conn, rebuild = FALSE, show_message = TRUE) {

  old <- options(list(stringsAsFactors = FALSE, scipen = 999))
  on.exit(options(old))
  valid_seasons <- load_phf_games() %>%
    dplyr::filter(.data$season >= 2016) %>%
  ...
  if (!is.null(seasons)) {
    ...
    load_phf_pbp(seasons, dbConnection = db_conn, tablename = tblname)
  }
}

# this is a helper function to check a list of completed games
# against the games that exist in a database connection
get_missing_phf_games <- function(completed_games, dbConnection, tablename) {
